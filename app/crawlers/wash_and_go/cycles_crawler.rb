module WashAndGo
  class CyclesCrawler < WashAndGo::BaseCrawler
    DOWNLOAD_FILE_FOLDER_PATH  = "./tmp/files/"
    class << self
      def run(store, email, password)
        @driver = setup_driver
        login(email, password)
        redirect_to_cycles_page(store)
        filter_cycles_by_date_range(store)
        download
        rename_file(store)
        puts "Downloaded"
      ...
      def redirect_to_cycles_page(store)
        @driver.get("#{ WashAndGo::BaseCrawler::URL}/admin/extratocondominio/#{store.external_id}")
        wait_until { page_loaded }
        puts "cycles - Page Loaded"
      end

      def filter_cycles_by_date_range(store)
        last_cycles = store.cycles.last
        puts "cycles - Updating Date Inputs"
        ...
        start_date, end_date = date_range(store, last_cycles)
        ...
        puts "cycles - Filtering Results for start date (#{start_date}) and end date(#{end_date})"
      ...
      def date_range(store, last_cycles)
        start_date = (last_cycles&.created_at&.strftime("%d/%m/%Y") || store.started_at.strftime("%d/%m/%Y"))
